...
import obj_pose_eval.misc as misc
import obj_pose_eval.visibility as visibility
...
def cou(pose_est, pose_gt, model, im_size, K):
    ...
    d_est = renderer.render(model, im_size, K, pose_est['R'], pose_est['t'],
                            clip_near=0.01, clip_far=100, mode='depth')
...
def re(R_est, R_gt):
    ...
    :return: Error of R_est w.r.t. R_gt.
    """
...
def reprojectionError(pose_est, pose_gt, model, K):
    """
    :param pose_est: Estimated pose given by a dictionary:
    {'R': 3x3 rotation matrix, 't': 3x1 translation vector}.
    :param pose_gt: The ground truth pose given by a dictionary (as pose_est).
    ...
    :return: Error of pose_est w.r.t. pose_gt.
    """
    Ximg_pix_est = np.dot(pose_est["R"], model['pts'].T) + pose_est["t"]
    Ximg_pix_est = Ximg_pix_est / Ximg_pix_est[2:3, :]
    Ximg_pix_est = np.dot(K, Ximg_pix_est)
    Ximg_pix_est = Ximg_pix_est[0:2, :]

    Ximg_pix_gt = np.dot(pose_gt["R"], model['pts'].T) + pose_gt["t"]
    Ximg_pix_gt = Ximg_pix_gt / Ximg_pix_gt[2:3, :]
    Ximg_pix_gt = np.dot(K, Ximg_pix_gt)
    Ximg_pix_gt = Ximg_pix_gt[0:2, :]

    reprojError = np.linalg.norm(Ximg_pix_est - Ximg_pix_gt, axis=0)
    reprojError = np.mean(reprojError)
    return reprojError
