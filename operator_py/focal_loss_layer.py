...
class FocalLoss(mx.operator.CustomOp):
    ...
    def backward(self, req, out_grad, in_data, out_data, in_grad, aux):
        ...
        '''
        cls_target = mx.nd.reshape(in_data[2], (0, 1, -1))  # batchsize x 1 x 8732
        p = mx.nd.pick(in_data[1], cls_target, axis=1, keepdims=True)   # batchsize x 1 x 8732

        n_class = in_data[0].shape[1]   # 2
        ...
        label_mask = mx.nd.one_hot(mx.nd.reshape(cls_target, (0, -1)), n_class,
                on_value=1, off_value=0)    # batchsize x 8732 x n_class
        label_mask = mx.nd.transpose(label_mask, (0, 2, 1)) # batchsize x n_class x 8732
