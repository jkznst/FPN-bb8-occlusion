...
def conv_act_layer(from_layer, name, num_filter, kernel=(1,1), pad=(0,0), \
    stride=(1,1), dilate=(1, 1), use_act=True, act_type="relu", use_batchnorm=False):
    """
    ...
    conv = mx.symbol.Convolution(data=from_layer, kernel=kernel, pad=pad, \
        stride=stride, dilate=dilate, num_filter=num_filter, name="{}_conv".format(name), bias=bias)
    if use_batchnorm:
...
def multi_layer_feature_SSD(body, from_layers, num_filters, strides, pads, min_filter=128):
    """Wrapper function to extract features from base network, attaching extra
    layers and SSD specific layers
...
def multibox_layer_retina(from_layers, num_classes, sizes=[.2, .95],
    ...
    bb8_conv1_weight = mx.symbol.Variable(name='bb8_conv1_weight',
                                          init=mx.init.Normal(sigma=0.01))
    bb8_conv1_bias = mx.symbol.Variable(name='bb8_conv1_bias',
                                        init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})

    bb8_conv2_weight = mx.symbol.Variable(name='bb8_conv2_weight',
                                          init=mx.init.Normal(sigma=0.01))
    bb8_conv2_bias = mx.symbol.Variable(name='bb8_conv2_bias',
                                        init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})

    bb8_conv3_weight = mx.symbol.Variable(name='bb8_conv3_weight',
                                          init=mx.init.Normal(sigma=0.01))
    bb8_conv3_bias = mx.symbol.Variable(name='bb8_conv3_bias',
                                        init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})

    bb8_conv4_weight = mx.symbol.Variable(name='bb8_conv4_weight',
                                          init=mx.init.Normal(sigma=0.01))
    bb8_conv4_bias = mx.symbol.Variable(name='bb8_conv4_bias',
                                        init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        bb8_conv1 = mx.symbol.Convolution(data=from_layer,
                                          weight=bb8_conv1_weight,
                                          bias=bb8_conv1_bias,
                                          kernel=(3, 3),
                                          stride=(1, 1),
                                          pad=(1, 1),
                                          num_filter=256,
                                          name="{}_bb8_conv1_3x3".format(from_name))
        bb8_conv1 = mx.symbol.Activation(data=bb8_conv1, act_type='relu')
        bb8_conv2 = mx.symbol.Convolution(data=bb8_conv1,
                                          weight=bb8_conv2_weight,
                                          bias=bb8_conv2_bias,
                                          kernel=(3, 3),
                                          stride=(1, 1),
                                          pad=(1, 1),
                                          num_filter=256,
                                          name="{}_bb8_conv2_3x3".format(from_name))
        bb8_conv2 = mx.symbol.Activation(data=bb8_conv2, act_type='relu')
        bb8_conv3 = mx.symbol.Convolution(data=bb8_conv2,
                                          weight=bb8_conv3_weight,
                                          bias=bb8_conv3_bias,
                                          kernel=(3, 3),
                                          stride=(1, 1),
                                          pad=(1, 1),
                                          num_filter=256,
                                          name="{}_bb8_conv3_3x3".format(from_name))
        bb8_conv3 = mx.symbol.Activation(data=bb8_conv3, act_type='relu')
        bb8_conv4 = mx.symbol.Convolution(data=bb8_conv3,
                                          weight=bb8_conv4_weight,
                                          bias=bb8_conv4_bias,
                                          kernel=(3, 3),
                                          stride=(1, 1),
                                          pad=(1, 1),
                                          num_filter=256,
                                          name="{}_bb8_conv4_3x3".format(from_name))
        bb8_conv4 = mx.symbol.Activation(data=bb8_conv4, act_type='relu')
        bb8_pred = mx.symbol.Convolution(data=bb8_conv4,
                                         weight=bb8_pred_weight,
                                         bias=bb8_pred_bias,
                                         kernel=(3, 3),
                                         stride=(1, 1),
                                         pad=(1, 1),
                                         num_filter=num_bb8_pred,
...
def multibox_layer_FPN(from_layers, num_classes, sizes=[.2, .95],
                    ratios=[1], normalization=-1, num_channels=[],
                    clip=False, interm_layer=0, steps=[]):
    """
    the basic aggregation module for SSD detection. Takes in multiple layers,
