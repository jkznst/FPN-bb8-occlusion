...
def load_COCO(COCO_path, nr_frames=0):

    COCO_images = []

    image_path = os.path.join(COCO_path, 'train2014/')

    train_name_file = open(os.path.join(COCO_path, 'trainNameList.txt'), 'r')
    train_name_list = train_name_file.readlines()
    # Load frames
    for i in tqdm(range(nr_frames)):
        fr = Frame()
        fr.nr = random.randint(0, len(train_name_list))

        fr.color = cv2.imread(image_path + train_name_list[fr.nr].strip('\n')).astype(np.float32) / 255.0

        COCO_images.append(fr)
